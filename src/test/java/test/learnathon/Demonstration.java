...
public class Demonstration implements Command {
	...
	public void run() {
		...
		parallelService.addProfile(new ParallelizationParadigmProfile(
			ImageJServerParadigm.class, "lonelyBiologist02"));
		parallelService.addProfile(new ParallelizationParadigmProfile(
			ImageJServerParadigm.class, "lonelyBiologist03"));
		...
		paradigm.parallelFor(angles, (angle, executionContext) -> {
			...
			executionContext.exportData(ds, Paths.get("/tmp/output/result_" + angle +
				".jpg"));
